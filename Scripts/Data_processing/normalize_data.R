...
#
# Reads <qc_dir>/<cohort>_qc.rds (res$rg from sample_qc()) and
# <qc_dir>/<cohort>_sample_qc.csv (qc_tab), drops samples flagged on
...
library(minfi)
source('/home/ec2-user/AMP-AD_methylation_harmonization/R/noob_normalize.R')
...
for (cohort_name in names(cohorts)) {
  ...
  qc_rds <- file.path(cfg$qc_dir, paste0(cohort_name, "_qc.rds"))
  qc_csv <- file.path(cfg$qc_dir, paste0(cohort_name, "_sample_qc.csv"))
